...
describe('Authenticated identity and access', () => {
  it('returns null without a session', async () => {
    vi.mocked(getSession).mockResolvedValue(null)
    expect(await getAuthUser()).toBeNull()
    expect(createRequestSdk).not.toHaveBeenCalled()
  })
  it('uses profile permissions when session claims differ', async () => {
  ...
  it('returns signed out when the profile request is unauthorized', async () => {
    vi.mocked(getSession).mockResolvedValue(createMockSession())
    profile({ code: 'unauthorized' }, 401)
    expect(await getAuthUser()).toBeNull()
  })
  it('redirects signed-out navigation to login with the encoded return path', async () => {
    vi.mocked(getSession).mockResolvedValue(null)
