...
**PRs as a request surface: no.** External pull requests are not feature-request intake for this repository.

GitHub shares one number space across issues and pull requests. Resolve an ambiguous reference with `gh pr view <number>`, then `gh issue view <number>` if it is not a pull request.
